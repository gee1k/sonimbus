...
- 精选歌单完整分类、热门/最新排序、排行榜、推荐音乐视频、新专辑、热门歌手
- 歌曲、歌单、专辑、歌手与音乐视频搜索，支持官方推荐词、最近搜索与连续分页
...
- 歌手热门歌曲、完整专辑目录、音乐视频与相似歌手推荐
- 个性化与最新音乐视频合并浏览，支持详情、内嵌预览、系统全屏时间轴、遥控器快进快退与上下文自动连播，自动选择最高可用清晰度
- LRC 同步歌词、翻译与罗马音，当前行自动滚动；遥控器可逐行选择并跳转播放
- Apple TV 遥控器焦点交互与系统 Now Playing / Remote Command 支持
- 选歌、全部播放或随机播放统一打开应用级全屏播放页，不切换底层内容 Tab，也不会在返回后自动弹回
- 播放页按“详情 → 待播清单 → 播放控制/歌词选择 → 来源页面”逐层返回，并保留原页面、筛选、滚动位置与歌曲焦点
